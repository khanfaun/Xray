...
import { ProjectData, LayerData, ProjectSummary } from '../types';
import { saveProject, getProjectsSummary, getProject, projectCache } from '../db';
import { ArrowLeft, ArrowUp, ArrowDown, Trash2, RefreshCw, Maximize, ChevronLeft, ChevronRight, Box, Settings2, Undo2, Redo2, AlignCenter, Scale, Eye, EyeOff, Sparkles, Play, Pause, Cpu } from 'lucide-react';
import AfterImageModal from './AfterImageModal';
import MetadataSidebar from './MetadataSidebar';
import { getImageDimensions } from '../utils/originalImageFinder';
...
function Thumbnail({ layer }: { layer: LayerData }) {
  ...
  useEffect(() => {
    const targetBlob = layer.thumbnailBlob || layer.blob;
    if (!(targetBlob instanceof Blob)) return;
    const objectUrl = URL.createObjectURL(targetBlob);
    setUrl(objectUrl);
    return () => URL.revokeObjectURL(objectUrl);
  }, [layer.thumbnailBlob, layer.blob]);
...
export default function XRayViewer({ initialProject, onBack }: { initialProject: ProjectData, onBack: () => void }) {
  ...
  const handleApplyAfterPair = async (origBlob: Blob | File, origName: string, afterF: File) => {
    ...
    // Auto-scale resize logic matching "dự án mới và upload ảnh":
    // Scale layer 2 (after) proportionally to match height of layer 1 (orig)
    let scaleRatio: number | undefined = undefined;
    try {
      const [origDim, afterDim] = await Promise.all([
        getImageDimensions(origBlob),
        getImageDimensions(afterF)
      ]);
      if (origDim.height > 0 && afterDim.height > 0) {
        scaleRatio = origDim.height / afterDim.height;
      }
    } catch (e) {
      console.warn('Could not precalculate dimensions in handleApplyAfterPair:', e);
    }

    const origLayer: LayerData = {
      id: `${timestamp}_orig_${Math.random().toString(36).substring(2, 7)}`,
      name: origName,
      type: 'image',
      blob: origBlob,
      layerScale: 1,
    ...
    const updatedP: ProjectData = {
      ...projectRef.current,
      name: `${origName.replace(/\.[^/.]+$/, '')} (AFTER + GỐC)`,
      layers: [origLayer, afterLayer],
    };
  ...
  const isPointerOverUIRef = useRef(false);
  ...
  useEffect(() => {
    ...
    // When project has 2 layers and depth is 0, default depth to 1 so the AFTER layer is displayed inside the lens
    if (project.layers.length >= 2 && viewState.current.depth === 0) {
      viewState.current.depth = 1;
    }
  ...
  useEffect(() => {
    ...
    project.layers.forEach(l => {
      if (l.blob instanceof Blob || l.blob instanceof File) {
        newUrls[l.id] = projectMediaCache.getUrl(l.id, l.blob);
      }
    });
    setUrls(newUrls);
    setLoadedCount(0);

    // Retain media URLs for active project layers + all cached recent projects
    const activeKeys = new Set<string>();
    project.layers.forEach(l => activeKeys.add(l.id));
    projectCache.getRecentProjects().forEach(p => {
      p.layers.forEach(l => activeKeys.add(l.id));
    });
    projectMediaCache.retainKeys(activeKeys);
  }, [blobsDep]);

  // Prefetch and warm up previous & next project in background for instant 0ms switching
  useEffect(() => {
    const currentIdx = allProjects.findIndex(p => p.id === project.id);
    const prevProj = currentIdx > 0 ? allProjects[currentIdx - 1] : null;
    const nextProj = currentIdx >= 0 && currentIdx < allProjects.length - 1 ? allProjects[currentIdx + 1] : null;

    const adjacentIds = [prevProj?.id, nextProj?.id].filter(Boolean) as string[];
    adjacentIds.forEach((id) => {
      getProject(id).then((proj) => {
        if (proj) projectMediaCache.preloadProject(proj);
      }).catch(() => {});
    });
  ...
  // Cleanly switch to an existing project without triggering an unnecessary auto-save
  const switchToProject = (target: ProjectData) => {
  ...
  const handleMatchLayerSize = () => {
    if (!selectedEditLayerIdRef.current) return;
    const selIdx = projectRef.current.layers.findIndex(l => l.id === selectedEditLayerIdRef.current);
    if (selIdx <= 0) return;

    const firstEl = elementsRef.current[0]?.el;
    const curEl = elementsRef.current[selIdx]?.el;
    if (!firstEl || !curEl) return;

    const firstH = firstEl instanceof HTMLVideoElement ? firstEl.videoHeight : (firstEl as HTMLImageElement).naturalHeight;
    const curH = curEl instanceof HTMLVideoElement ? curEl.videoHeight : (curEl as HTMLImageElement).naturalHeight;
    if (!(firstH > 0 && curH > 0)) return;

    pushHistory(projectRef.current);
    const ratio = firstH / curH;
    const newLayers = JSON.parse(JSON.stringify(projectRef.current.layers));
    newLayers[selIdx].layerScale = ratio;
    newLayers[selIdx].layerScaleX = ratio;
    newLayers[selIdx].layerScaleY = ratio;
    newLayers[selIdx].layerX = 0;
    newLayers[selIdx].layerY = 0;
    updateProject({ ...projectRef.current, layers: newLayers });
  };
  ...
  const getMasterDimensions = () => {
    // Layer 0 is the primary base canvas / reference layer (Layer 1 in UI)
    const baseEl = elementsRef.current[0]?.el;
    if (baseEl) {
      const w = baseEl instanceof HTMLVideoElement ? baseEl.videoWidth : (baseEl as HTMLImageElement).naturalWidth;
      const h = baseEl instanceof HTMLVideoElement ? baseEl.videoHeight : (baseEl as HTMLImageElement).naturalHeight;
      if (w > 0 && h > 0) {
        return { imgW: w, imgH: h };
      }
    }
    // Fallback if base layer is not yet loaded
    let mw = 0;
    let mh = 0;
  ...
  useEffect(() => {
    ...
    const visW = (imgW || 1) * lScaleX0;
    const visH = (imgH || 1) * lScaleY0;

    const cw = canvas.width;
    const ch = canvas.height;
    // Fit scale: prioritizes expanding height close to full application height (~96%) while staying inside frame without cropping
    const maxW = cw * 0.98;
    const maxH = ch * 0.96;
    const fitScale = Math.min(maxW / visW, maxH / visH) || 1;
    
    if (viewState.current.scale === 0) {
      viewState.current.scale = fitScale;
      viewState.current.offsetX = (cw - visW * fitScale) / 2;
      viewState.current.offsetY = (ch - visH * fitScale) / 2;
    }
    ...
    apiRef.current = {
      ...
      fitScreen: () => {
        ...
        const { imgW: curW, imgH: curH } = getMasterDimensions();
        const lData0 = projectRef.current.layers[0];
        const lScaleX0 = lData0?.layerScaleX ?? lData0?.layerScale ?? 1;
        const lScaleY0 = lData0?.layerScaleY ?? lData0?.layerScale ?? 1;
        const visW = (curW || 1) * lScaleX0;
        const visH = (curH || 1) * lScaleY0;

        const curCw = canvas.width;
        const curCh = canvas.height;
        const curMaxW = curCw * 0.98;
        const curMaxH = curCh * 0.96;
        const newFitScale = Math.min(curMaxW / visW, curMaxH / visH) || 1;
        viewState.current.scale = newFitScale;
        viewState.current.offsetX = (curCw - visW * newFitScale) / 2;
        viewState.current.offsetY = (curCh - visH * newFitScale) / 2;
        setZoomLevel(newFitScale * 100);
      }
    };
    ...
    const updateHUD = () => {
      ...
      if (viewState.current.viewMode === 'compare') {
        if (depthHud) depthHud.innerText = `So sánh: Lớp ${viewState.current.compareBefore + 1} ↔ Lớp ${viewState.current.compareAfter + 1} (${Math.round(viewState.current.comparePos * 100)}%)`;
      } else if (viewState.current.autoAnimate) {
        const fromL = Math.floor(viewState.current.depth) + 1;
        const toL = Math.min(L, Math.ceil(viewState.current.depth) + 1);
        if (depthHud) depthHud.innerText = `Phát mượt: Lớp ${fromL} ⇄ ${toL} (${L > 1 ? Math.round((viewState.current.depth / (L - 1)) * 100) : 0}%)`;
      } else {
    ...
    const draw = () => {
      ...
      if (isEditModeRef.current) {
        ...
        // Draw ultra-thin hairline split line in screen space ONLY if cursor is inside image area!
        ...
        const isCursorInImage = isInsideImage && !isPointerOverUIRef.current && isMouseInsideCanvasRef.current;
        if (isCursorInImage) {
          const lineX = Math.round(canvas.width * vs.comparePos) + 0.5;
          const screenTop = Math.max(0, lTop0 * vs.scale + vs.offsetY);
          const screenBottom = Math.min(canvas.height, lBottom0 * vs.scale + vs.offsetY);
        ...
        // Lens mode OR AutoAnimate full-image mode (Play mượt lớp)
        if (vs.autoAnimate) {
          // Play mượt lớp: The main image smoothly plays and transitions between layers without needing a lens!
          const floorIdx = Math.min(L - 1, Math.floor(vs.depth));
          const ceilIdx = Math.min(L - 1, Math.ceil(vs.depth));
          const floorEl = elementsRef.current[floorIdx]?.el;
          const ceilEl = elementsRef.current[ceilIdx]?.el;

          if (floorIdx === ceilIdx || floorIdx >= L - 1) {
            ctx.globalAlpha = 1.0;
            if (floorEl) drawLayer(ctx, floorEl, floorIdx);
          } else {
            // Base layer with 100% opacity
            ctx.globalAlpha = 1.0;
            if (floorEl) drawLayer(ctx, floorEl, floorIdx);

            // Crossfading layer on top with transition progress opacity
            const progress = Math.max(0, Math.min(1, vs.depth - floorIdx));
            if (progress > 0 && ceilEl) {
              ctx.globalAlpha = progress;
              drawLayer(ctx, ceilEl, ceilIdx);
            }
          }
        } else {
          // Lens mode: base layer is firstEl (Layer 0)
          if (firstEl) {
            ctx.globalAlpha = 1.0;
            drawLayer(ctx, firstEl, 0);
          }

          if (isInsideImage && !isPointerOverUIRef.current) {
            ctx.beginPath();
            if (vs.lensShape === 1) {
      ...
      if (!isEditModeRef.current) {
        ...
        const isOverUI = target ? (
          target !== canvas && (
            ...
            target.closest('.floating-controls') !== null ||
        ...
        } else if (viewState.current.enable3D && viewState.current.viewMode === 'lens') {
          canvas.style.cursor = 'default';
    ...
    const onCanvasLeave = () => {
      isPointerOverUIRef.current = true;
  ...
  return (
    <div className="h-screen bg-[#050608] text-[#e2e8f0] font-sans flex flex-col overflow-hidden">
      ...
      <header 
        ...
        className="h-14 border-b border-white/10 bg-[#0a0c10] flex items-center justify-between px-6 shrink-0 z-20"
      >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-mono tracking-wider transition-all border ${
              isMetadataOpen
                ? 'bg-cyan-500/20 text-cyan-300 border-cyan-400/50 font-bold shadow-[0_0_10px_rgba(34,211,238,0.2)]'
                : 'bg-white/5 text-slate-400 border-white/10 hover:text-white'
            }`}
            title="Bật/tắt cột Metadata AI bên trái"
          >
            <Cpu size={12} className={isMetadataOpen ? 'text-cyan-400' : 'text-slate-500'} />
          ...
          <button
            ...
            title="Bật/tắt cột Điều khiển bên phải"
          >
            <Settings2 size={12} className={isRightSidebarOpen ? 'text-cyan-400' : 'text-slate-500'} />
            <span className="hidden sm:inline">ĐIỀU KHIỂN</span>
          </button>

          <div className="hidden md:flex items-center gap-6 text-[11px] font-mono tracking-wider opacity-60 ml-2">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {/* Left Column: AI Metadata Inspector */}
        <MetadataSidebar
        ...
        {/* Right Column: Layer & Effect Controls */}
        {!isRightSidebarOpen ? (
          <aside 
            onMouseEnter={() => { isPointerOverUIRef.current = true; }}
            onMouseLeave={() => { isPointerOverUIRef.current = false; }}
            className="w-10 border-l border-white/5 bg-[#08090c] flex flex-col items-center py-4 z-20 shrink-0 select-none"
          >
            <button
              onClick={() => setIsRightSidebarOpen(true)}
              className="p-2 text-slate-400 hover:text-cyan-400 hover:bg-white/5 rounded-lg transition-colors flex flex-col items-center gap-2 cursor-pointer"
              title="Mở Bảng Điều Khiển Lớp & Hiệu Ứng"
            >
              <Settings2 size={18} className="text-cyan-400" />
              <span className="text-[9px] uppercase tracking-widest [writing-mode:vertical-lr] font-mono text-slate-400 font-bold mt-2">
                ĐIỀU KHIỂN
              </span>
            </button>
          </aside>
          ...
          <aside 
            onMouseEnter={() => { isPointerOverUIRef.current = true; }}
            onMouseLeave={() => { isPointerOverUIRef.current = false; }}
            className="w-[280px] max-w-[320px] border-l border-white/10 bg-[#08090c] flex flex-col z-20 shrink-0 overflow-hidden"
          >
            {/* Top Header matching left Metadata column */}
            <div className="p-3.5 border-b border-white/10 flex items-center justify-between bg-[#0a0c10]">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="truncate">
                  ...
                  <p className="text-[9px] font-mono text-slate-400">Lớp & Hiệu ứng</p>
                </div>
              </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-cyan-500/10 border border-cyan-400/40 hover:bg-cyan-500/20 text-cyan-300 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2 mb-2 shadow-[0_0_12px_rgba(34,211,238,0.15)]"
              title="Upload ảnh AFTER và tự động đọc metadata để tìm ảnh gốc"
            >
              <Sparkles size={13} className="text-cyan-400" />
              Ghép Ảnh AFTER (Auto-Detect)
            </button>
              ...
              <div className="pt-4 border-t border-white/5 space-y-5 mb-6">
                 <div>
                   ...
                   <div className="flex gap-2 mb-4">
                     ...
                     <button onClick={handleAlignCenter} disabled={!selectedEditLayerId} className="flex-1 py-1.5 flex items-center justify-center gap-1 text-[9px] uppercase tracking-widest rounded-sm transition-colors border bg-transparent text-slate-400 border-white/10 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed" title="Căn giữa lớp đang chọn">
                       <AlignCenter size={12}/> Giữa
                     </button>
                     <button onClick={handleMatchLayerSize} disabled={!selectedEditLayerId || selectedEditLayerId === project.layers[0]?.id} className="flex-1 py-1.5 flex items-center justify-center gap-1 text-[9px] uppercase tracking-widest rounded-sm transition-colors border bg-transparent text-cyan-400/80 border-cyan-400/20 hover:bg-cyan-500/10 disabled:opacity-30 disabled:cursor-not-allowed" title="Tự động co giãn theo tỉ lệ chiều cao lớp gốc (Layer 1)">
                       <Scale size={12}/> Khớp Size
                     </button>
                   </div>
                ...
                <>
                  ...
                  {viewState.current.viewMode === 'compare' && (
                    <div className="space-y-4">
                      ...
                      <div className="p-2.5 bg-cyan-500/10 border border-cyan-400/20 rounded text-[9px] text-cyan-300 leading-relaxed font-mono">
                        💡 <b>Tự động di chuyển</b>: Di chuột trái/phải trên ảnh, thanh kéo sẽ tự động trượt theo mà không cần bấm giữ chuột (chuẩn ComfyUI)!
                      </div>
                    </div>
                  )}
                  ...
                  {viewState.current.viewMode === 'lens' && (
                    <>
                      ...
                      <div>
                        <h2 className="text-[10px] uppercase tracking-[0.2em] text-slate-500 mb-3 mt-4">Play Mượt Toàn Ảnh</h2>
                        <div className="flex gap-2 items-center mb-4">
                          <button 
                            onClick={() => updateVS('autoAnimate', !viewState.current.autoAnimate)} 
                            className={`flex-1 py-2 text-[10px] uppercase tracking-widest border rounded transition-all cursor-pointer flex items-center justify-center gap-2 ${
                              viewState.current.autoAnimate 
                                ? 'bg-red-500/15 text-red-400 border border-red-500/40 shadow-[0_0_10px_rgba(239,68,68,0.2)]' 
                                : 'bg-cyan-500/15 text-cyan-300 border border-cyan-500/30 hover:bg-cyan-500/25 shadow-[0_0_10px_rgba(34,211,238,0.2)]'
                            }`}
                          >
                            {viewState.current.autoAnimate ? <Pause size={12} className="fill-current text-red-400" /> : <Play size={12} className="fill-current text-cyan-400" />}
                            <span className="font-bold">{viewState.current.autoAnimate ? 'Dừng mượt' : 'Play mượt lớp'}</span>
                          </button>
                        </div>
                        {viewState.current.autoAnimate && (
                           <div className="animate-in fade-in slide-in-from-top-2">
                              <div className="flex justify-between text-[9px] font-mono text-slate-400 mb-2">
                                <span>TỐC ĐỘ CHUYỂN</span>
                              </div>
                              <div className="flex gap-1">
                                <button onClick={() => updateVS('autoAnimateSpeed', 0.006)} className={`flex-1 py-1.5 text-[9px] uppercase tracking-widest rounded-sm transition-colors border cursor-pointer ${viewState.current.autoAnimateSpeed <= 0.008 ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50 font-bold' : 'bg-transparent text-slate-400 border-white/10 hover:bg-white/5'}`}>Chậm</button>
                                <button onClick={() => updateVS('autoAnimateSpeed', 0.012)} className={`flex-1 py-1.5 text-[9px] uppercase tracking-widest rounded-sm transition-colors border cursor-pointer ${viewState.current.autoAnimateSpeed > 0.008 && viewState.current.autoAnimateSpeed <= 0.016 ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50 font-bold' : 'bg-transparent text-slate-400 border-white/10 hover:bg-white/5'}`}>Vừa</button>
                                <button onClick={() => updateVS('autoAnimateSpeed', 0.024)} className={`flex-1 py-1.5 text-[9px] uppercase tracking-widest rounded-sm transition-colors border cursor-pointer ${viewState.current.autoAnimateSpeed > 0.016 ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50 font-bold' : 'bg-transparent text-slate-400 border-white/10 hover:bg-white/5'}`}>Nhanh</button>
                              </div>
                           </div>
                        )}
                      </div>
      ...
      <AfterImageModal
        ...
        onClose={() => setIsAfterModalOpen(false)}
        onProjectReady={(newP) => {
          updateProject(newP);
          updateVS('depth', 1);
          updateVS('viewMode', 'lens');
        }}
        onApplyLayersToCurrentProject={handleApplyAfterPair}
      />
    </div>
  );
}
